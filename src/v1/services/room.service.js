...
const createRoom = async ({ ownerId, displayName, isGroupChat = false, memberIds = [], byPass = false }) => {
    try {
        ...
        if (!ownerId || !displayName || !memberIds.length)
            return { isCreateSuccess: false, message: 'Invalid params!', httpCode: 400 };
        ...
        const status = await addToUserRoom(ownerId, memberIds, newRoom, false);
...
const checkPermissionToAction = (actionType, userId, roomId) => {
    return new Promise((resolve, reject) => {
        ...
        switch (actionType) {
            ...
            default:
                return reject({ status: false, message: 'Invalid type!', httpCode: 400 });
        }
    });
};
...
const getOnlineStatusRoom = async (userId) => {
    return new Promise((resolve, reject) => {
        ...
        if (!room) return reject({ name: 'Error:', message: `Missing status room with id:${userId}` });
        return resolve(room);
    });
};
        ...
        return UserModel.find({ _id: { $in: objectIds } })
            .then(async (users) => {
                ...
                if (!users?.length)
                    return resolve(
                        memberIds.map((id) => {
                            return { id, code: 404, message: 'Cannot found this user.', name: 'Not Found' };
                        }),
                    );
                const usersMap = {};
                users.forEach((user) => {
                    ...
                    memberIds.map((id) => {
                        return new Promise(async (resolve, reject) => {
                            ...
                            if (!isNeedToAcceptToJoinRoom(userDetail['settings'], relationship, bypass)) {
                                ...
                                room.members.set(id, { user_id: id, last_active: lastActive, is_online: isOnline });
                            } else {
                ...
                await room.save();
                return resolve({ code: 200, message: 'Success!', name: '200', payload: result });
            })
            .catch((error) => {
                return reject(error);
            });
