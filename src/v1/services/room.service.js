...
const joinRoomByRoomId = async ({
  ...
  try {
    ...
    const status = await addToUserRoom(
      requestSendBy,
      memberIds,
      existRoom,
      byPass
          ...
          memberIds.map((id) => {
            return new Promise(async (resolve, reject) => {
              ...
              const relationship = isMyFriend(
                id === request_send_by ||
                  userDetail["friend_details"]["accepted_list"].has(
                    request_send_by
                  ),
                true
              );
              if (
                !isAllowAddToRoom(userDetail["settings"], relationship, bypass)
              )
                return resolve({
                  id,
                  code: "USER_PREVENT",
              ...
              if (
                ...
                userDetail.room_details.message_room["accepted_list"].set(
                  String(room.room_id),
                  {
                ...
                userDetail.room_details.message_room["waiting_accept_list"].set(
                  String(room.room_id),
                  {
                    user_id: new Types.ObjectId(request_send_by),
                    room_id: room.room_id,
