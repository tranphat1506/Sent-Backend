...
const { ENDPOINTS } = require("../routers/socketio.router");
...
const RoomEventBySocketIO = (socket) => {
  ...
  // Event create room
  socket.on(
    "create-room__Request",
    async ({ members = [], display_name, is_group_chat = false }) => {
      // Create room
      roomService
        .createRoom({
          ownerId: _id,
        ...
        .then(async (response) => {
          if (response.isCreateSuccess) {
            ...
            response.payload.user_status.forEach((status) => {
    ...
    .then(({ payload }) => {
      ...
      return {
        ...
        joinPromises: Promise.all(
          Object.keys(payload["messagesRooms"]).map((roomId) =>
            joinRoomById(roomId, socket)
          )
        ),
      };
    })
