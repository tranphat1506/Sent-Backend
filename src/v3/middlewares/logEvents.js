...
const logEvents = async (message, typeLog) => {
    ...
    try {
        ...
        return await fsPromise.appendFile(
            path.join(__dirname, '../../../', 'logs', typeLog, currentDate + '.txt'),
            logItem,
        );
    } catch (error) {
        console.log(error);
...
const errorHandle = (error, req, res, next) => {
    process.env.NODE_ENV != 'development'
        ? logEvents(
              `${req.method}\t${req.headers.origin || 'localhost'}\t${req.url}\t${error.name}: ${error.message}`,
              `errors`,
          )
        : console.log(
              `${req.headers.origin || 'localhost'} ${req.method} ${req.path}\t${error.name}: ${error.message}`,
          );
