...
const createRoom = async ({ ownerId, displayName, isGroupChat = false, memberIds = [], byPass = false }) => {
    try {
        if (!ownerId || !displayName) return ROOM_ACTION_STATUS.ROOM_COMMON__INVALID_PARAMS;
...
const sentInviteJoinRoomByRoomId = async ({ requestSendBy, roomId, memberIds = [], byPass = false }) => {
    try {
        ...
        if (existRoom === null) return ROOM_ACTION_STATUS.GET_ROOM__FAILED;
        //Không thể thêm vào phòng nếu không phải là group chat
        if (!existRoom.is_group_chat) return ROOM_ACTION_STATUS.ADD_TO_ROOM__CANNOT_ADD_TO_ONE_ONE_ROOM;
    ...
    return new Promise((resolve, reject) => {
        ...
        switch (actionType) {
            case 'MESSAGE_TEXT':
                ...
                // Maybe lỗi server
                if (!_MESSAGE_ROOMS[roomId])
                    return reject({
                        status: false,
                        message: 'Server is busy!',
                        httpCode: 500,
                    });
                if (!_MESSAGE_ROOMS[roomId].members.get(userId))
                    return reject({
                        status: false,
...
const getUserJoinedRooms = (userId) => {
    return new Promise((resolve, reject) => {
        if (!userId || !isObjectIdOrHexString(userId)) return resolve(ROOM_ACTION_STATUS.ROOM_COMMON__INVALID_PARAMS);
...
const inviteUsersToMessageRoom = async (request_send_by, memberIds = [], room, bypass = false) => {
    return new Promise((resolve, reject) => {
        if (!memberIds || !memberIds.length || !room)
                ...
                const result = await Promise.all(
                    memberIds.map((memberId) => {
                        return new Promise(async (resolve, reject) => {
                            ...
                            if (!userSetting)
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__USER_NOT_FOUND,
                                    user_id: memberId,
                                });
                            // If user already in group
                            if (userSetting.response_invite === 'Accepted') {
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__USER_ALREADY_JOIN,
                                    user_id: memberId,
                                });
                            }
                            ...
                            if (userSetting.response_invite === 'Pending') {
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__ALREADY_INVITE,
                                    user_id: memberId,
                                });
                            }
                            const relationship = isMyFriend(isSelf || userSetting.is_friend, true);
                            // Check if user allow you to add to the room
                            if (!isAllowAddToRoom(userSetting, relationship, bypass))
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__USER_NOT_ALLOW,
                                    user_id: memberId,
                                });
                            ...
                            if (!isNeedToAcceptToJoinRoom(userSetting, relationship, bypass)) {
                                ...
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__SUCCESS_JOIN_ROOM,
                                    user_id: memberId,
                                });
                            } else {
                                ...
                                await newMember.save();
                                return resolve({
                                    ...ROOM_ACTION_STATUS.ADD_TO_ROOM__SUCCESS_SENT_INVITE,
                                    user_id: memberId,
                                });
                            }
                        });
